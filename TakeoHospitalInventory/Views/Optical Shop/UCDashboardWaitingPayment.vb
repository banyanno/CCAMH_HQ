...
Public Class UCDashboardWaitingPayment
    ...
    Private Sub BtnCancelReceipt_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnCancelReceipt.Click
        If GridReceipt.SelectedItems.Count = 0 Then
            Exit Sub
        Else
            If MessageBox.Show("Do you want to cancel this receipt?", "Cancel", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = System.Windows.Forms.DialogResult.Yes Then
    ...
    Private Sub BtnUndoReceipt_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnUndoReceipt.Click
        If GridReceipt.SelectedItems.Count = 0 Then
            Exit Sub
        Else
            If MessageBox.Show("Do you want to undo this receipt?", "Undo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = System.Windows.Forms.DialogResult.Yes Then
                ObjReceip = ReceiptDataContext.GetReceipt(GridReceipt.GetRow.Cells("ReceiptID").Value)
                ObjReceip.IsPaid = 0
                If ReceiptDataContext.UpdateReceiptPay = True Then
    ...
    Private Sub BtnApproveNilInvoice_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnApproveNilInvoice.Click
        ...
        Else
            ...
            If MessageBox.Show("Do you want to approve NIL Invoice?", "Approve", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = System.Windows.Forms.DialogResult.Yes Then
                Dim ReceiptDataContext As IOpticalShop = New OpticalshopImp
                Dim ObjReceip As New RECEIPT
                ...
                If ReceiptDataContext.UpdateReceiptPay = True Then
                    ...
                    Try
                        ...
                        For Each recDetail As RECEIPT_DETAIL In listReceiptDetail.ToList
                            ...
                            Dim qDepartStock = From departStock In THIDataContext.getTHIDataContext.tblDeptStocks Where departStock.DepartID = SALE_DEPART_ID And departStock.ItemID = itemID
                            unitsInStock = qDepartStock.SingleOrDefault.UnitsInStock

                            Dim q = (From BBT In THIDataContext.getTHIDataContext.tblBeginBalanceTraces Where BBT.Date.Value.Date = GetDateServer.Date And BBT.DepartID = SALE_DEPART_ID And BBT.ItemID = itemID Select BBT.BeginBalanceTraceID).Count
                            If q = 0 Then
                                Dim mytblBeginBalanceTrace As New tblBeginBalanceTrace
                                mytblBeginBalanceTrace.Date = GetDateServer()
                                mytblBeginBalanceTrace.DepartID = SALE_DEPART_ID
