Public Class frmRequestItemsDetail
    ...
    Dim TableUse As New DSRequest.VUsedWithUsedDetailDataTable
            ...
            If isRequestStoreItemInCabinet = False Then
                ...
                If ChItemUsed.Checked = True Then
                    Try
                        ...
                        Else
                            If MessageBox.Show("Do you want add these item to list?", "add", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = System.Windows.Forms.DialogResult.Yes Then
                                For Each row As Janus.Windows.GridEX.GridEXRow In checkedRows
                                    ...
                                    dataRow("cInStock") = 0
                                    dataRow("Quantity") = (CInt(row.Cells("TotalItemUsed").Value)) ' + ((CInt(row.Cells("TotalItemUsed").Value) * CInt(LblAddMorePercent.Text) / 100))
                                    dataRow("Barcode") = row.Cells("Barcode").Value
                                    dataRow("RequestItemnote") = ""
                    ...
                    Else
                        If MessageBox.Show("Do you want add these item to list?", "add", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = System.Windows.Forms.DialogResult.Yes Then
                            For Each row As Janus.Windows.GridEX.GridEXRow In checkedRows
                                ...
                                dataRow("ItemID") = row.Cells("ItemID").Value
                                dataRow("ItemName") = row.Cells("ItemName").Value
                                dataRow("RequestQuantity") = (NearestHundred((CInt(row.Cells("TotalItemUsed").Value) + ((CInt(row.Cells("TotalItemUsed").Value) * CInt(LblAddMorePercent.Text) / 100))))) - row.Cells("UnitsInStock").Value
                                dataRow("Barcode") = row.Cells("Barcode").Value
    ...
    Private Sub ChItemUsed_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ChItemUsed.CheckedChanged
        'SplitContainer1.Panel2.Enabled = ChItemUsed.Checked
    End Sub
End Class