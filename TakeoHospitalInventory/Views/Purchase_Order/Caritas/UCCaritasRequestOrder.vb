...
Public Class UCCaritasRequestOrder
    ...
    Dim DA_CorrentStock As DSHospitalRequestToCaritasTableAdapters.tblCentralInventoryTableAdapter

 

    Private Sub subMenuRequestNewItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
    ...
    Private Sub menuRecRequestItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles menuRecRequestItem.Click
        ...
        Catch ex As Exception
            MessageBox.Show(ex.Message & " Please select main request order ", "Order", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    Private Sub menuApprovedRecRequest_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles menuApprovedRecRequest.Click
        ...
        Try
            If Not gridCaritasRequestOrder123.CurrentRow Is Nothing AndAlso gridCaritasRequestOrder123.CurrentRow.RowType = RowType.Record Then
                ...
                Dim requestID As Double = gridCaritasRequestOrder123.GetValue("RequestID")

                '--- Update item quantity in stock
                If IsRequestExistingItem = True Then
                    'ApprovedRecReqExistItem(requestID)
                    If MessageBox.Show("Do you want save approved Request Order ?", "Save", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = System.Windows.Forms.DialogResult.Yes Then
        ...
        Try
            ...
            For Each CaritasReqDetail As CARITAS_REQUEST_DETAIL_INVENTORY In lstCaritasReqDetail
                ...
                If CaritasReqDetail.IsReceive = True And CaritasReqDetail.IsReady = False Then
                    ...
                    Dim balanceStock As Integer = 0
                    Try
                        balanceStock = centralStock.UnitsInStock + recQuantity
                    Catch ex As Exception
                        balanceStock = recQuantity
                    End Try
    ...
    Private Sub menuDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles menuDelete.Click
        ...
        Try
            If Not gridCaritasRequestOrder123.CurrentRow Is Nothing AndAlso gridCaritasRequestOrder123.CurrentRow.RowType = RowType.Record Then
                ...
                If MessageBox.Show("Do you want delete Request Order ?", "Save", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = System.Windows.Forms.DialogResult.Yes Then
    ...
    Private Sub BtnItemSupUsed_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnItemSupUsed.Click
        CaristasRequestMain.FillByReceivedBy(DSHosRequestCaritas.CARITAS_REQUEST_INVENTORY, CboSupplier.Text)
        CaristasRequestDetail.Fill(DSHosRequestCaritas.CARITAS_REQUEST_DETAIL_INVENTORY)
        gridCaritasRequestOrder123.DataSource = DSHosRequestCaritas.CARITAS_REQUEST_INVENTORY
        gridCaritasRequestOrder123.RootTable.ChildTables(0).DataMember = "CARITAS_REQUEST_INVENTORY_CARITAS_REQUEST_DETAIL_INVENTORY"
    ...
    Private Sub BgRunReceived_RunWorkerCompleted(ByVal sender As System.Object, ByVal e As System.ComponentModel.RunWorkerCompletedEventArgs) Handles BgRunReceived.RunWorkerCompleted
        ...
        'DEP_EOD.BtnEndofDay_Click(sender, e)
        MessageBox.Show("Please run EOD to completed data sumarry!", "EOD", MessageBoxButtons.OK, MessageBoxIcon.Information)
    End Sub
    ...
    Private Sub BtnReceiveMoreItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnReceiveMoreItem.Click
        Try
            If Not gridCaritasRequestOrder123.CurrentRow Is Nothing AndAlso gridCaritasRequestOrder123.CurrentRow.RowType = RowType.Record Then
                Dim FRecievedMoreItem As New FormReceiveMoreExistingItem
                FRecievedMoreItem.LblRequestID.Text = gridCaritasRequestOrder123.GetValue("RequestID")
                FRecievedMoreItem.txtReOrNum.Text = gridCaritasRequestOrder123.GetValue("RequestOrderNumber")
                FRecievedMoreItem.lbUserRequest.Text = IUser.GetUserNameByID(CInt(gridCaritasRequestOrder123.GetValue("UserID")))
                FRecievedMoreItem.dtpRDate.Value = gridCaritasRequestOrder123.GetValue("RequestDate")
                ' =============== Show item received detial ==========================


                If FRecievedMoreItem.ShowDialog = DialogResult.OK Then
                    ' menuApprovedRecRequest_Click(sender, e)
                    BtnFilter_Click(sender, e)
                End If
            End If
        Catch ex As Exception
    ...
    Private Sub PrintReportDrugReceivedToolStripMenuItem1_Click(sender As Object, e As EventArgs) Handles PrintReportDrugReceivedToolStripMenuItem1.Click
        Try
            If Not gridCaritasRequestOrder123.CurrentRow Is Nothing AndAlso gridCaritasRequestOrder123.CurrentRow.RowType = RowType.Record Then
                '---------- Update ending balance stock ..........................
                '---- print request order ----------
                Dim FRptRequestView As New frmReportViewer
                FRptRequestView.CaritasRequestID = CInt(gridCaritasRequestOrder123.GetValue("RequestID"))
                FRptRequestView.ReportOption = 26
                FRptRequestView.ShowDialog()
                '  FillByRequestID(CInt(gridCaritasRequestOrder123.GetRow.Cells("RequestID").Value))
            End If
        Catch ex As Exception
